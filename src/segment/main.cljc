(ns segment.main
  "Kotodama WASM entrypoint for the Segment clean-room actor (L4) — Clojure port.
  ...
  facts (`segment.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "segment")
(def tier "L4")
...
(def entity-specs
  [{:entity "TrackEvent"   :plural "trackevents"   :id-prefix "segment_tra"
    :fields [:type :event :userId :anonymousId :messageId :timestamp] :required [:type :event]
    :coerce {}                       :refs {}}
   {:entity "IdentifyEvent" :plural "identifyevents" :id-prefix "segment_ide"
    :fields [:type :userId :anonymousId :messageId :timestamp] :required [:type :timestamp]
    :coerce {}                       :refs {}}
   {:entity "PageEvent"    :plural "pageevents"    :id-prefix "segment_pag"
    :fields [:type :userId :anonymousId :category :name :messageId :timestamp] :required [:type :category]
    :coerce {}                       :refs {}}
   {:entity "GroupEvent"   :plural "groupevents"   :id-prefix "segment_gro"
    :fields [:type :userId :anonymousId :groupId :messageId :timestamp] :required [:type :timestamp]
    :coerce {}                       :refs {}}
   {:entity "Source"       :plural "sources"       :id-prefix "segment_sou"
    :fields [:name :slug :enabled :workspaceId]     :required [:name :slug]
    :coerce {:enabled :bool}         :refs {}}
   {:entity "Destination"  :plural "destinations"  :id-prefix "segment_des"
    :fields [:name :enabled :sourceId :workspaceId] :required [:name :enabled]
    :coerce {:enabled :bool}         :refs {:sourceId "Source"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"segment.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "segment-compat" :tier tier :entities entities} 200])
...
(defn start! [] :segment-compat/ready)
